<x-layouts::app :title="__('Tambah Kategori')">
    <div class="flex flex-col gap-6 w-full">
        <div class="flex items-center justify-between">
            <div>
                <flux:heading size="xl" level="1">{{ __('Tambah Kategori') }}</flux:heading>
            </div>
        </div>

        <flux:separator variant="subtle" />

        <form action="{{ route('admin.category.store') }}" method="POST" class="flex flex-col gap-6 w-full max-w-4xl mt-2">
            @csrf

            <div class="grid grid-cols-1 gap-6">

                <flux:input name="category_name" :label="__('Nama kategori')" type="text" autofocus
                    :placeholder="__('Masukkan nama kategori')" value="{{ old('category_name') }}" />
            </div>
